...
namespace RhinoScriptEncrypter
{
  ...
  /// </summary>
  [System.Runtime.InteropServices.Guid("679017E7-4C0A-4607-AA70-0CDEEED69BE7")]
  public class PythonScriptEncrypterCommand : Command
  {
    int _optionIndex = 0;
    ...
    /// Public constructor
    /// </summary>
    public PythonScriptEncrypterCommand()
    {
    }
    ...
    {
      get { return "PythonScriptEncrypter"; }
    }
    ...
    protected override Result RunCommand(RhinoDoc doc, RunMode mode)
    {
      ...
      if (mode == RunMode.Interactive)
      {
        ...
        openFileDialog.Filter = "PythonScript Files (*.py)|*.py";
        openFileDialog.Title = "Select PythonScript File";
      ...
      {
        Result rc = GetOpenFileName("PythonScript file to encrypt", ref rhinoScriptFile);
      ...
      {
        string error = string.Format("PythonScript file not found - {0}\n", rhinoScriptFile);
        if (mode == RunMode.Interactive)
      ...
      try
      {
        ...
        encryptedFileName = Path.ChangeExtension(rhinoScriptFile, ".pyx");
        File.WriteAllText(encryptedFileName, encryptedString);
      }
      catch (Exception ex)
      {
        string error = string.Format("Error encrypting PythonScript file - {0}\n{1}\n", rhinoScriptFile, ex.Message);
        if (mode == RunMode.Interactive)
          MessageBox.Show(error, EnglishName, MessageBoxButtons.OK, MessageBoxIcon.Error);
        else
          RhinoApp.WriteLine(error);
      ...
      // Report the results
      string outputString = "PythonScript file encryption successful!\n";
    ...
    private Result GetOpenFileName(string prompt, ref string fileName)
    {
      ...
      else if (res == GetResult.Option)
      {
        ...
        fileDialog.Filter = "RhinoScript Files (*.py)|*.py";
        fileDialog.Title = "Select PythonScript File";
